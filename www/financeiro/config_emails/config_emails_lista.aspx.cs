namespace MedProj.www.financeiro.config_emails
{
    ...
    public partial class config_emails_lista : System.Web.UI.Page
    {
        ...
        {
            List<ConfigEmailAviso> lista = null;
            ...
            if (cboAssociadoPJ.SelectedIndex <= 0 && cboContratoADM.SelectedIndex <= 0) //if (cboAssociadoPJ.SelectedIndex < 0 && cboContratoADM.SelectedIndex <= 0)
                lista = ConfigEmailFacade.Instancia.Carregar();
                ...
                lista = ConfigEmailFacade.Instancia.CarregarPorAssoc(CTipos.CToLong(cboAssociadoPJ.SelectedValue));
                ...
                lista = ConfigEmailFacade.Instancia.CarregarPorAssoc(CTipos.CToLong(cboAssociadoPJ.SelectedValue), CTipos.CToLong(cboContratoADM.SelectedValue));
            else if (cboContratoADM.SelectedIndex <= 0 && cboContrato.SelectedIndex == 0) //selecionou um contrato adm, mas todos os contratos
                lista = ConfigEmailFacade.Instancia.CarregarPor(CTipos.CToLong(cboContratoADM.SelectedValue));
            else if (cboContratoADM.SelectedIndex > 0 && cboContrato.SelectedIndex > 0)
                lista = ConfigEmailFacade.Instancia.CarregarPor(CTipos.CToLong(cboContratoADM.SelectedValue), CTipos.CToLong(cboContrato.SelectedValue));
        ...
        {
            Response.Redirect("config_emails.aspx");
        }
        ...
        protected void grid_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("Editar"))
            {
                ...
                Response.Redirect(string.Format("config_emails.aspx?{0}={1}", Util.Keys.IdKey, id));
            }
        ...
        protected void lnkTextos_Click(object sender, EventArgs e)
        {
