    ...
    public partial class cancelar : System.Web.UI.Page
    {
        ...
        protected void grid_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("detalhe"))
            {
                ...
                //gridDetalhe2.DataSource = procedimentos;
                //gridDetalhe2.DataBind();


                //litTotal.Text = string.Concat("Total: ", procedimentos.Sum(p => p.Valor).ToString("C"));
        ...
        {
            List<Ent.AtendimentoProcedimento> procedimentos = 
                AtendimentoCredFacade.Instance.CarregarProcedimentosDoAtendimento(this.IdAtendimento);

            if (procedimentos == null || procedimentos.Count == 0)
            {
                Util.Geral.Alerta(null, this, "_err", "Nenhum procedimento localizado para esse atendimento.");
                return;
            }
        ...
        protected void gridDetalhe2_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("Excluir"))
            {
                try
                {
                    long usuid  = Util.CTipos.CToLong(Util.UsuarioLogado.ID);
                    long procid = Util.Geral.ObterDataKeyValDoGrid<long>(gridDetalhe2, e, 0);

                    Ent.AtendimentoProcedimento proc = AtendimentoCredFacade.Instance.CarregarProcedimentosDeAtendimento(procid);

                    if (MedProj.www.Util.UsuarioLogado.TipoUsuario != Util.UsuarioLogado.Tipo.Administrador)
                    {
                        if (proc.Atendimento.Data.Day != DateTime.Now.Day ||
                            proc.Atendimento.Data.Month != DateTime.Now.Month ||
                            proc.Atendimento.Data.Year != DateTime.Now.Year)
                        {
                            Util.Geral.Alerta(null, this, "_err", "Não é permitido cancelar atendimentos de datas anteriores.");
                            return;
                        }
                    }
                    ...
                    if (proc.Cancelado)
                    {
                        Util.Geral.Alerta(null, this, "_err", "Procedimento já cancelado.");
                        return;
                    }

                    AtendimentoCredFacade.Instance.CancelarProcedimento(procid, usuid);

                    this.carregarDetalhe();
                }
                catch
                {
                    Util.Geral.Alerta(null, this, "_err", "Não foi possível cancelar o procedimento.");
                }
            }
        }

        protected void gridDetalhe2_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                Util.Geral.grid_AdicionaToolTip<LinkButton>(e, 4, 0, "Cancelar");
            ...
            Util.Geral.grid_RowDataBound_Confirmacao(sender, e, 4, "Deseja cancelar o procedimento?\\n\\nEssa operação não poderá ser desfeita.");
        }

        protected void cmdFecharPnl_Click(object sender, EventArgs e)
        {
            this.fecharDetalhe();
        }
    }
